...
export class PublicProfileService {
    ...
    getAvailableUsers(): Observable<PublicUserResultClass[]> {
        return this.http.get<NotLoadedPublicUserResult[]>('/public-profile').pipe(map((users) => {
            return users.map((user) => new PublicUserResultClass(user, this.http));
        }));
    }

    getMatches(): Observable<PublicUserResultClass[]> {
        return this.http.get<NotLoadedPublicUserResult[]>('/public-profile/matches').pipe(map((users) => {
            return users.map((user) => new PublicUserResultClass(user, this.http));
        }));
